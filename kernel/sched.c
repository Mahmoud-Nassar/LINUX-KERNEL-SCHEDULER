...
struct runqueue {
    ...
    task_t *curr, *idle;
	/********************* hw2 **************************/
    prio_array_t *active, *sc_active,*expired, arrays[3];
...
/********************************** hw2 ****************************/ 
	if (p->policy == SCHED_CHANGEABLE&&p->state == TASK_RUNNING) {
        runqueue_t *rq = this_rq();
		spin_lock_irq(rq);
        list_add_tail(&p->sc_run_list, rq->sc_active->queue);
		spin_unlock_irq(rq);
    }
    rq->nr_running++;
...
static int try_to_wake_up(task_t *p, int sync) {
    ...
    if (!p->array) {
         ...
         */
/******************************* hw2 ****************************/
		 if (rq->regime_on&& rq->curr->policy == SCHED_CHANGEABLE) {
            if ((p->prio < rq->curr->prio) ||(p->policy == SCHED_CHANGEABLE && p->pid < rq->curr->pid)) {
                goto needreschedOn;
            }
...
void scheduler_tick(int user_tick, int system) {
    ...
    spin_lock(&rq->lock);
    /************************* hw2 *****************************/ 
    if (rq->regime_on&&current->policy == SCHED_CHANGEABLE) {			
		spin_unlock(&rq->lock);
		return;
    }
...
int get_sc_count(){
    runqueue_t* rq=this_rq();
    spin_lock_irq(rq);
    int count=rq->sc_count;
    spin_unlock_irq(rq);
    return count;
}

int getMinID(task_t *task) {
    runqueue_t *rq = this_rq();
    int min_id = task->pid;
    struct list_head *tmp;
    list_for_each(tmp, rq->sc_active->queue){
        task_t *tsk = list_entry(tmp, task_t, sc_run_list);
        if (tsk->pid < min_id)
            min_id = tsk->pid;
    }
    return min_id;
...
void fork_helper(struct task_struct *tsk) {
   if (tsk->state == TASK_RUNNING) {
        runqueue_t *rq = this_rq();
		spin_lock_irq(rq);
        list_add_tail(&tsk->sc_run_list, rq->sc_active->queue);
		spin_unlock_irq(rq);
         }
}
...
int sys_make_changeable(pid_t pid) {
    ...
    if (current->policy == SCHED_CHANGEABLE || task->policy == SCHED_CHANGEABLE) {
        return -EINVAL;
    }
    ...
    spin_lock_irq(rq);
    task->policy = SCHED_CHANGEABLE;
	// time to add the proccess to the DS
   if (task->state == TASK_RUNNING) {
        list_add_tail(&task->sc_run_list, rq->sc_active->queue);
         }
    if (getMinID(task) == task->pid&&rq->regime_on) {
...
asmlinkage void schedule(void) {
	...
	/****************** hw2 ********************************/
    if (next->policy == SCHED_CHANGEABLE) {
        while (next->policy == SCHED_CHANGEABLE && rq->regime_on&& getMinID(next) < next->pid) {
...
/******************************* hw2 ***********************************/
    runqueue_t *rq = this_rq_lock();
    if (current->policy == SCHED_CHANGEABLE && rq->regime_on) {
        spin_unlock(&rq->lock);
		return 0;
    }
